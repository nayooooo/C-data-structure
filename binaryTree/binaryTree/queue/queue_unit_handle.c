...
#if QUEUE_UNIT_USE_16BIT

queue_err_t _qu_malloc_16bit(void** queue_buff, queue_size_t size)
{
	*queue_buff = obj_malloc(size * sizeof(queue_uint16_t));

	return QUEUE_EOK;
}

queue_err_t _qu_free_16bit(void* block)
{
	if (block == QUEUE_NULL) return -QUEUE_PARAM;
	obj_free(block);
...
)
{
	...
	dst_addr = &(((queue_uint16_t*)dst_base)[dst_offset]);
	src_addr = &(((queue_uint16_t*)src_base)[src_offset]);
	true_size = size * sizeof(queue_uint16_t);
	obj_memcpy(dst_addr, src_addr, true_size);
...
#endif
#if QUEUE_UNIT_USE_16BIT
	{ QUEUE_UNIT_TYPE_16BIT, _qu_malloc_16bit, _qu_free_16bit, _qu_memcpy_16bit },
#endif
#if QUEUE_UNIT_USE_32BIT
	...
	while (quh->type != QUEUE_NULL) {
		...
		if (ret == 0) {
			...
			if (handle == QUEUE_UNIT_HANDLE_MALLOC) {
				...
				if (queue_buff == QUEUE_NULL) goto queue_unit_handler_param_error;
				if (size <= 0) goto queue_unit_handler_param_error;
				quh->qu_malloc(queue_buff, size);
			}
			else if (handle == QUEUE_UNIT_HANDLE_FREE) {
				void* block = va_arg(ap, int);
				if (block == QUEUE_NULL) goto queue_unit_handler_param_error;
				quh->qu_free(block);
			}
				...
				queue_size_t size = va_arg(ap, queue_size_t);
				if (dst_base == QUEUE_NULL) goto queue_unit_handler_param_error;
				if (src_base == QUEUE_NULL) goto queue_unit_handler_param_error;
				if (dst_offset < 0) goto queue_unit_handler_param_error;
				if (src_offset < 0) goto queue_unit_handler_param_error;
				if (size <= 0) goto queue_unit_handler_param_error;
			...
			va_end(ap);
			if (0) {
			queue_unit_handler_param_error:
				va_end(ap);
				return -QUEUE_PARAM;
			}
			return QUEUE_EOK;
